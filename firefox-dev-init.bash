...
# The proper name of the package, for this version of Firefox, Developer
...
if [ ! -r $MOZ_LIB_DIR/$PACKAGE_NAME/$MOZ_FIREFOX_FILE ]; then
	if [ ! -r $SECONDARY_LIB_DIR/$PACKAGE_NAME/$MOZ_FIREFOX_FILE ]; then
		echo "Error: $MOZ_LIB_DIR/$PACKAGE_NAME/$MOZ_FIREFOX_FILE not found"
		if [ -d $SECONDARY_LIB_DIR ]; then
			echo "       $SECONDARY_LIB_DIR/$PACKAGE_NAME/$MOZ_FIREFOX_FILE not found"
		fi
		exit 1
	fi
	MOZ_LIB_DIR="$SECONDARY_LIB_DIR"
fi
MOZ_DIST_BIN="$MOZ_LIB_DIR/$PACKAGE_NAME"
...
##
export MOZ_APP_LAUNCHER="/usr/bin/$PACKAGE_NAME"
...
if ! [ $MOZ_DISABLE_LANGPACKS ] || [ $MOZ_DISABLE_LANGPACKS -eq 0 ]; then
    if [ -x $MOZ_DIST_BIN/$MOZ_FIREFOX_FILE ]; then
        # Is firefox running?
        /usr/bin/pidof $PACKAGE_NAME > /dev/null 2>&1
        MOZILLA_DOWN=$?
    fi
fi
